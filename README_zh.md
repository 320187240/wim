<div align="center">

# [![wim](https://cdn.jsdelivr.net/gh/320187/wim/src/main/resources/META-INF/pluginIcon@30x30.svg)](https://github.com/320187/wim)
**wim**

[English](./README.md) / 简体中文

知游工具，一款基于 IntelliJ IDEA 的集成工具插件。它汇集了日常开发中常用的各种功能，旨在帮助开发者简化重复工作、提高代码开发效率和质量。

[![Homepage](https://img.shields.io/badge/Homepage-wim-0db7ed.svg?style=Plastic&logo=data:image/svg+xml;base64,)](https://plugins.jetbrains.com/plugin/24381-wim)
&nbsp;
[![Release](https://img.shields.io/github/v/release/320187/wim?style=Plastic&label=Release&color=d05ce3&logo=data:image/svg+xml;base64,)](https://github.com/320187/wim/releases)
&nbsp;
[![GitHub Actions Workflow Status](https://img.shields.io/github/actions/workflow/status/320187/wim/verification.yml?branch=main&style=plastic&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D&label=Build)](https://github.com/320187/wim/actions/workflows/Compatibility.yml)
&nbsp;
[![Java](https://img.shields.io/badge/Lang-Java-ff5722.svg?style=Plastic&logo=data:image/svg+xml;base64,)](https://www.oracle.com/cn/java/)
&nbsp;
[![IntelliJ IDEA](https://img.shields.io/badge/Jetbrains-IntelliJ%20IDEA-%2311AB00.svg?style=Plastic&logo=intellij-idea&logoColor=white)](https://www.jetbrains.com/?from=wim)

</div>

<br/>

## 功能 🎉
#### 代码生成与辅助

- [Json转JavaBean](https://GBox.320187.cn/json-to-javabean.html)：Json 反序列化为 JavaBean，支持嵌套 Array、Object 属性。
- [JavaBean转Json](https://GBox.320187.cn/javabean-to-json.html)：JavaBean 序列化为 Json，支持嵌套 Object 属性，支持
  FastJson、Jackson 注解。
- [ResultMap标签生成](https://GBox.320187.cn/generates-resultmap-tag.html)：通过 SQL 与 Java 类属性映射生成
  MyBatis-ResultMap 标签。
- [Where标签生成](https://GBox.320187.cn/generates-where-tag.html)：通过 Java 类属性生成 MyBatis-Where 条件标签。
- [类属性映射与方法列举](https://GBox.320187.cn/attributes-mapping.html)：类与类之间属性匹配，单独类的列举 Getter、Setter
  方法等功能。
- [JDK序列化ID生成](https://GBox.320187.cn/generate-serialization-id.html)：为 Java 类生成 JDK序列化ID，并实现序列化接口。
- [MyBatis-Mapper文件生成](https://GBox.320187.cn/generate-mybatis-files.html)：生成 Mapper
  接口、Mapper-Xml、映射类等文件，并自动匹配文件路径。
- [指定注解、注释生成](https://GBox.320187.cn/generate-annotation.html)：为类属性生成指定注解及注释。

#### 代码与文本格式转换

- [单行注释转Java文档注释](https://GBox.320187.cn/convert-to-javadoc.html)
- [字符串格式转换](https://GBox.320187.cn/convert-strings.html)：包含驼峰、下划线互转，中文转拼音功能，支持多行多文本转换。
- [时间戳与时间转换](https://GBox.320187.cn/convert-timestamp-to-date.html)：快速转换时间戳 / 时间。
- [JSON/XML转换](https://GBox.320187.cn/convert-json-to-xml.html)：快速转换 Json / XML。

#### 拓展功能

- [代码笔记功能](https://GBox.320187.cn/code-note.html)：记录代码片段、笔记、备忘，支持标签、导入、导出等功能。
- [文本互相比对](https://GBox.320187.cn/text-comparison.html)：精准辨识文本的不同之处。
- [Json窗口及Json处理](https://GBox.320187.cn/json-panel.html)：支持 Json格式化、Json压缩、Json结构化。
- [Jasypt加解密](https://GBox.320187.cn/jasypt-encrypt-and-decrypt.html)：Jasypt 加密、解密功能，支持对称加密、非对称加密。
- [Spring接口路径提取](https://GBox.320187.cn/copy-restful-path.html)：快速复制 Spring 接口路径。

<br/>

## 快速开始 🚀

> 导航：[使用文档](https://GBox.320187.cn/overview.html)

<br/>


运行
gradlew runIde

## 开源许可证

[![Licence](https://img.shields.io/github/license/320187/wim?style=for-the-badge)](LICENSE)

<br/>

## 插件截图

![Json Panel](https://web-images-5we.pages.dev/images/JsonPanel.jpg)

![MyBatis](https://web-images-5we.pages.dev/images/MyBatis.jpg)

![Mapping](https://web-images-5we.pages.dev/images/Mapping.gif)


